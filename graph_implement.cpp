...
// backup Dijkstra code
vector<int> FatherList;
typedef pair<int, ALArc *> path;
vector<path> PathList;
int find_father(int x)
{
    if (FatherList[x] != x)
        FatherList[x] = find_father(FatherList[x]);
    return FatherList[x];
}
static inline bool path_cmd(path a, path b)
{
    return a.second->weight < b.second->weight;
}
void ALGraph::Kruskal(int strtID)
{
    ...
    {
        PathList[i].second->gArc->visit(false);
    }
    PathList.clear();
    FatherList.clear();
    for (int i = 0; i < vexList.size(); i++)
    {
        ...
        for (ALArc *p = vexList[i].firstArc; p != nullptr; p = p->nextArc)
        {
            PathList.push_back({i, p});
        }
    }
    sort(PathList.begin(), PathList.end(), path_cmd);
    for (int i = 0; i < PathList.size(); i++)
    {
        int st = PathList[i].first, ed = PathList[i].second->eVexID, w = PathList[i].second->weight;
        int father_st = find_father(st), father_ed = find_father(ed);
        if (father_ed != father_st)
        {
            if (!vexList[st].visited)
                vexList[st].visit();
            FatherList[father_st] = father_ed;
            if (type == UDG && GetIdOf(PathList[i].second->gArc->edVex()) != ed)
            {
                PathList[i].second->gArc->reverseDirection();
                ans += w;
            }
            PathList[i].second->visit();
            if (!vexList[ed].visited)
                vexList[ed].visit();
        }
    }
}
    ...
    while (true)
    {
        ...
        if (edge)
        {
            if (type == UDG && GetIdOf(edge->gArc->edVex()) != minVexID)
                edge->gArc->reverseDirection(); // (i find GUI!!)   draw the line !!  by lyz
            edge->visit();
        }
...
void AMLGraph::Kruskal(int strtID)
{
    // building....
}
