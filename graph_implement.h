...
public:
    ...
    virtual void Kruskal(int strtID) = 0;
    virtual void Kruskal(MyGraphicsVexItem *strtVex) = 0;
...
public:
    ...
    void Kruskal(int strtID);
    void Kruskal(MyGraphicsVexItem *strtVex) { Kruskal(GetIdOf(strtVex)); }
...
public:
    ...
    AMLArc(MyGraphicsLineItem *garc, int sVex, int eVex, AMLArc *nextOut, AMLArc *nextIn) : gArc(garc), outVexID(sVex), nextOutArc(nextOut), inVexID(eVex), nextInArc(nextIn) {}
    void visit();
    void access();
};
